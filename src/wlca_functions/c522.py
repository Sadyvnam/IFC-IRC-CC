...
def category_522(ad: CoreType, idx: PreIndex):
    out = CategoryReturn()
    out.issues = []
    out.categories = []
    out.overview = IndicatorRow()
    out.overview.category = "5.2.2"

    # --- 1) systems (preferred) ---
    cooling_sys_types = {"AIRCONDITIONING", "CHILLEDWATER", "CONDENSERWATER", "REFRIGERATION"}
    systems = [s for s in ad.by_types(["IfcDistributionSystem"])
               if getattr(s, "PredefinedType", None) in cooling_sys_types]

    if not systems:
        out.issues.append(IssueRow(
            category_code="5.2.2", category_name="", ifc_class="IfcDistributionSystem",
            message="No cooling system found (expected explicit IfcDistributionSystem cooling PredefinedType)"
        ))
        finalize_category_metrics(ad, out, {"5.2.2": []})
        return out

    # --- 2) members via IfcRelAssignsToGroup ---
    members = []
    if systems:
        sys_set = set(systems)
        for rel in ad.by_types(["IfcRelAssignsToGroup"]):
            ...
            if grp in sys_set:
                members += list(getattr(rel, "RelatedObjects", []))

    # --- 3) no class-only fallback ---
    used_fallback = False
    if not members:
        out.issues.append(IssueRow(
            category_code="5.2.2", category_name="", ifc_class="IfcRelAssignsToGroup",
            message="Cooling system exists but has no members assigned via IfcRelAssignsToGroup"
        ))
        finalize_category_metrics(ad, out, {"5.2.2": []})
        return out

    generation_classes = {
        "IfcChiller", "IfcCoolingTower", "IfcEvaporativeCooler",
        "IfcCompressor", "IfcCondenser", "IfcEvaporator",
    }
    def classify(el) -> str:
        return "5.2.2.1" if el.is_a() in generation_classes else "5.2.2.2"

    # --- 4) aggregate + indicators (combined across 5.2.2.*) ---
    ...
    finalize_category_metrics(
        ad,
        out,
        {"5.2.2": scoped_elements},
        qto_by_category={"5.2.2": relevant_qto_types},
    )
